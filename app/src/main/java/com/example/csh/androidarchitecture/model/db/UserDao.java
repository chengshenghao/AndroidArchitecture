...
public interface UserDao {
    ...
    @Query("SELECT * FROM users WHERE uid = 11")
    List<User> getuid();

    @Query("SELECT * FROM users WHERE firstName LIKE :select")
    List<User> getfirstName(String select);

    @Query("update users set street=:stree where uid=:i")
    void update(String stree,int i);
